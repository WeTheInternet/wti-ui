...
class TaskFactoryTest extends Specification {
    ...
    def "TaskManager prints accurate human readable deadlines for ONCE recurrence"() {
        ...
        ModelTask task = TaskFactory.create("ONCE TEST")
        ...
        then:
        TaskFactory.nextTime(task).longValue() == amount
    }

    def "TaskManager prints accurate human readable deadlines for WEEKLY recurrence"() {
        when:
        ModelTask task = TaskFactory.create("WEEKLY TEST")
        ...
        task.recurrence().add(recur)
        then:
        new Date(TaskFactory.nextTime(task).longValue()) == new Date(1000 * later.toEpochSecond(ModelSettings.timeZone()))
