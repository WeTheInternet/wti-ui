...
public class EntityBrowserActor<E> extends Table implements Disposable {
    ...
    private boolean doubleClickActivates;
    private int columns;
    ...
    /// Enables or disables activation on the second click of a double-click.
    ///
    /// The first click still performs selection only. The default is false so
    /// browse-only consumers do not acquire an activation gesture implicitly.
    public void setDoubleClickActivates(final boolean doubleClickActivates) {
        this.doubleClickActivates = doubleClickActivates;
    }

    /// Returns whether a double-click emits explicit activation.
    public boolean isDoubleClickActivates() {
        return doubleClickActivates;
    }
    ...
    private void attachSelection(final Actor cell, final String stableKey) {
        cell.addListener(new ClickListener() {
            ...
            public void clicked(final InputEvent event, final float x, final float y) {
                handleCellClick(stableKey, getTapCount());
            }
        });
    }

    void handleCellClick(final String stableKey, final int tapCount) {
        model.setSelectedKey(stableKey);
        requestBrowserFocus();
        if (doubleClickActivates && tapCount == 2) {
